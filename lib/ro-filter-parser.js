...
export function setFilter(definition, { label, subtype, values }) {
  ...
  parsedFilter.forEach((filter, index) => {
    if (parsedFilter[index].array) {
      const cleanArray = parsedFilter[index].array.map(f => cleanObject(f)).filter(f => !isEmpty(f));
  ...
  parsedFilter = parsedFilter.map(f => cleanObject(f)).filter(filter => !isEmpty(filter));
